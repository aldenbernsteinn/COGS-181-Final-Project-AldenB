...
\textbf{(a) Architecture matters most.} YOLOv8s (11.2M parameters) jumps to mAP50=0.134 from the near-zero YOLOv8n baseline (0.002, which reflects the severe class imbalance in the 5-class formulation). YOLOv8m (25.9M) does not improve further despite 2.3$\times$ more parameters---on limited data, bigger models hit diminishing returns fast.

\textbf{(b) More layers do not help.} Doubling C2f depth produces 0.107 mAP50---no better than the baseline-depth model trained from scratch (0.108), but slower (28ms vs.\ 19ms CPU). Extra layers add cost without benefit on this dataset.
...
\textbf{(d) Max pooling slightly outperforms average pooling.} AvgPool2d in SPPF yields 0.106 vs.\ 0.108 with MaxPool2d. Max pooling preserves sharp features like edges of buttons and input fields. The difference is small (0.002 mAP50).

\textbf{(e) SiLU slightly outperforms ReLU.} Replacing SiLU with ReLU drops mAP50 from 0.108 to 0.103. SiLU's smooth gradient helps optimization---consistent with why modern architectures default to it over ReLU.

\textbf{Best combo.} Combining winners (YOLOv8s + AdamW + lr=0.001 + 1280px + no mosaic) gives only 0.136---just +0.002 over standalone YOLOv8s. Combining the best settings from each axis yields only marginal additional gains.

\textbf{Per-class breakdown.} Table~\ref{tab:yolo_perclass} reveals why: only \texttt{interactive} (0.375) and \texttt{form\_input} (0.288) classes are detectable. The other three classes are nearly zero regardless of hyperparameters, which led us to try consolidating to a single class.
...
Motivated by the ablation's per-class findings (Table~\ref{tab:yolo_perclass}) and OmniParser's single-class approach~\cite{omniparser}, we consolidated all annotations to one \texttt{ui\_element} class and applied two cleaning steps:
...
Table~\ref{tab:data_scaling} shows the result. The same YOLOv8n model that scored 0.002 on 5-class data now achieves \textbf{0.323 mAP50} on 1-class cleaned data---a 2.4$\times$ improvement over our best 5-class experiment (YOLOv8s at 0.134) after reformulating the task and improving data quality. Recall jumps from 0.005 to 0.426.
...
The initial 16-class formulation proved too granular for our data---the per-class analysis from the ablation is what revealed this, and restructuring into a single detection class with diverse sampling resolved it.
...
\caption{\textbf{Left:} Validation mAP50 over 25 training epochs. The 1-class diverse model converges to 0.323, while the 5-class baseline remains near zero throughout. YOLOv8s and the best combination reach similar plateaus ($\sim$0.13). \textbf{Right:} Training box loss decreases for all configurations, confirming that all models are learning---the 5-class baseline's low mAP50 reflects task formulation issues rather than optimization failure.}
