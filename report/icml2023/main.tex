...
\begin{document}
\renewcommand{\icmlconference}{COGS 181 Final Project, UC San Diego, Winter 2026}
...
\textbf{Backbone (CSPDarknet53).} Convolutional layers with Cross-Stage Partial connections extract features at multiple scales through stride-2 downsampling and \textbf{C2f blocks}---residual bottleneck modules whose count is controlled by a \textit{depth multiplier}. All convolutions use \textbf{SiLU} activation by default ($x \cdot \sigma(x)$, a smooth alternative to ReLU).

\textbf{Neck (Feature Pyramid Network).} Multi-scale features are fused via upsampling and concatenation, similar to how skip connections in ResNet preserve spatial information across depths.

\textbf{Head + SPPF.} Detection heads predict boxes and classes at three scales. The \textbf{SPPF module} applies cascaded \texttt{MaxPool2d} for multi-scale context---we test \texttt{AvgPool2d} as an alternative.

The model family spans nano (3.2M params), small (11.2M), and medium (25.9M) variants.

\subsection{Training Setup}

\textbf{Hardware.} All training on an NVIDIA RTX 5090 GPU.
...
\textbf{Metrics.} mAP50, mAP50-95, precision, recall, CPU latency (ms), model size (MB).
...
\textbf{(e) SiLU beats ReLU.} Replacing SiLU with ReLU drops mAP50 from 0.108 to 0.103. SiLU's smooth gradient helps optimization---consistent with why modern architectures default to it over the simpler ReLU we used in HW4.
...
\textbf{What we learned about CNNs.} The homework assignments had us training simple CNNs on balanced datasets like CIFAR-10, where architecture and hyperparameter tuning make clear differences. On our real-world crawled data, those same knobs (pooling, activation, depth) barely moved the needle compared to data quality. This was surprising and is probably the most useful takeaway: on a new detection task with noisy labels, fix the data first.
