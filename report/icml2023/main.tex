...
Since OmniParser~\cite{omniparser} also uses a single detection class, we consolidated all annotations to one \texttt{ui\_element} class and applied two cleaning steps:
...
\textbf{(a) Novel application: prompt injection classifier.} While YOLO can verify what elements exist in the visible frame, LLM-powered agents can also read elements that are off-screen or hidden in the DOM---meaning text-based attacks can bypass visual detection entirely. This makes text classification a natural next step for filtering harmful content. As a separate experiment, we trained a DeBERTa-v3-xsmall~\cite{deberta} text classifier (22M parameters) for detecting prompt injection attacks in UI element text. The model uses focal loss~\cite{focalloss} ($\gamma=2.0$, $\alpha=[0.7,0.3]$)---a variant of cross-entropy that down-weights easy examples so the model focuses on hard cases. We trained on 23K balanced examples from six public sources and ran 17 ablation experiments across architecture, loss function, class weighting, sequence length, and data composition. Table~\ref{tab:classifier_benchmarks} compares against two published baselines on independent benchmarks with zero training data overlap.
